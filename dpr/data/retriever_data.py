...
class JsonLawQASrc(QASrc):
    def __init__(
        self,
        file: str,
        selector: DictConfig = None,
        question_attr: str = "q_short", # TODOgao
        answers_attr: str = "answers", # 没有answer
        id_attr: str = "ridx",
        ridx: int = -1,
        special_query_token: str = None,
        query_special_suffix: str = None,
    ...
    def load_data(self):
        if self.ridx == -1:
            print("Please specify ridx in config file")
            return
        super().load_data()
        qa = []
        with open(self.file, "r") as f:
            data = json.load(f)
        for _dict in data:
            if "ridx" in _dict and _dict["ridx"] == self.ridx:
                question = _dict[self.question_attr]
                answers = [_dict[self.answers_attr]] if self.answers_attr in _dict else []
                id = _dict[self.id_attr]
                qa.append(QASample(self._process_question(question), id, answers))
                break
        if not qa:
            print("No such ridx in data")
            return
        self.data = qa
...
class JsonLawCtxSrc(RetrieverData):
    def __init__(
        self,
        query_ridx_path: str,
        id_prefix: str = None,
        ...
        super().__init__(query_ridx_path)
        self.id_prefix = id_prefix
        self.normalize = normalize
    ...
    def load_data_to(self, ctxs: Dict[object, BiEncoderPassage]):
        ...
        for file in os.listdir(self.file):
            with open(os.path.join(self.file, file)) as f:
                data = json.load(f)
                if self.id_prefix:
                    sample_id = self.id_prefix + file[:-5]
                else:
                    sample_id = file[:-5]
                passage = data["ajjbqk"]
                if self.normalize:
                    passage = normalize_passage(passage)
                ctxs[sample_id] = BiEncoderPassage(passage, data["ajName"])
