...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        # model is HFBertEncoder
        # retrieve encoded contents of the passages.
        with torch.no_grad():
            _, out, _ = model(ctx_ids_batch, ctx_seg_batch, ctx_attn_mask)
        out = out.cpu() #pooled output
...
#     checkpoint_file_name=dpr_biencoder -> model_file=
#     output_dir=${output_checkpoint_dir} \


@hydra.main(config_path="conf", config_name="gen_embs")
def main(cfg: DictConfig):
    # cfg.model_file = "/mnt/d/model_train/law_model/dpr_biencoder.1"
    # cfg.ctx_src = "todo" # TODO
    # cfg.out_file = "todo"
    # setattr(cfg.encoder, "pretrained_model_cfg", "/mnt/d/hf/Lawformer")
    # setattr(cfg.encoder, "sequence_length", 256)
    # # cfg.encoder["pretrained_model_cfg"] = "/mnt/d/hf/Lawformer"
    # # cfg.encoder["sequence_length"] = 256
    # cfg.fp16 = True
    # override shard_id, num_shards
    ...
    # refer to BiencoderTrainer constructor.
    # we do not need optimizer.
    ...
    # region setup_model
    # refer to train_epoch in training.
    # load weights from the model file

    # what immediately done after member fields initializations.
    ...
    # refer to readme: name of the passages resource, set to dpr_wiki to use wiki split.
    # ctx_sources is defaulted to default_sources
    # which is in conf/ctx_sources/default_sources.yaml
    # target is dpr.data.retriever_data.CsvCtxSrc

    # the purpose of ctx_src is to fill up all_passages_dict only.
    all_passages_dict = {}
    if cfg.base_path:
        import gen_px_ctx  # type: ignore
        all_passages_dict = gen_px_ctx.gen_doc_dict(cfg.base_path)
    else:
...
# shard_id is used for slicing passages processing, which means 
# when capacity = n(passages) / shard_size,
# each instance process for passage id [shard_id * capacity, (shard_id + 1) * capacity)
